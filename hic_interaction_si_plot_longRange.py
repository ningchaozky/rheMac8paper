...
parser.add_argument( '-span', nargs = '?', help = 'span for plot defalut 100', type = int, default = 50 )
parser.add_argument( '-cut', nargs = '?', help = 'cut for the postive bins', type = float, default = 3 )
...
def parse(fl, peirod ):
    ...
    for line in fh:
        ...
        # reverse the value
        wbin = abs(int(wbin) - args.span - 1 )
        trick.set3dict(dit, peirod, chrom, wbin, [ float(shape), float(scale) ])
    fh.close()
    return dit

def make_signal( dirs, quiet, needLoadChroms):
    ...
    for peirod in signal_file_dit:
        for chrom in chroms:
            if chrom not in needLoadChroms:
                continue
            ...
            ofl = '/'.join([work_dir, fix.fix( peirod ).append('%s.LongRange.hic.%s.signal' % (chrom, str(args.span)))])
            flag = '%s.LongRange' % os.path.basename( ofl )
            trick.set2dict( dit, peirod, chrom, ofl)
            if flag not in stat or not os.path.exists(ofl) or os.stat(ofl).st_size == 0 or line_num.line_num(ofl) != args.span :
                ofh = open(ofl,'w')
...
def hic_dit(chroms, quiet):
    dit = {}
    signal_file_dit = make_signal( dirs, quiet, chroms)
    for peirod in signal_file_dit:
        for chrom in chroms:
...
def bline_pick_pvalue( bline, signal, rs, array, genome):
    ...
    if bline.strip():
        ...
        for pos in values:
            value = values[pos]
            bg_pos = pos + 1
            if bg_pos < args.span + 1 :
                bg_pos = args.span + 1 - bg_pos
            elif bg_pos > args.span + 1 :
                bg_pos = bg_pos - args.span - 1
            else :
                bg_pos = 1
            for peirod in peirods:
                trick.set1dict(pvalues, peirod, [])
                if bg_pos == args.span + 1 :
                    pvalues[peirod].append( 1 )
                    continue
...
def b2span(bline, rs, genome):
    ...
    if sbin > max_bin:
        ...
        return []
    lst = list(range(-args.span, 0, 1))
    lst.extend(list(range(0, args.span + 1, 1)))
    pbin = [ i + sbin for i in lst ]
    span = [ i for i in pbin if i >= 0  and i < max_bin ]
    coord = [ [ sbin , i  ] for i in span ]
    return coord

def tab_postive_regions( tab, cut ):
    fh, dit, out, pdit = open( tab ), {}, '', {}
    header = next( fh ).strip('\n').split('\t')
    for line in fh:
        line_arr = line.strip('\n').split('\t')
        line_arr[3:] = [ float(i) for i in line_arr[3:] ]
        tdit = dict(zip(header,line_arr))
        chrom, start, end = trick.dit( tdit ).get(['chr','start','end'], delete = True )
        for peirod in tdit:
            if tdit[peirod] > cut :
                trick.dinit( dit, peirod, [])
                dit[peirod].append( '\t'.join( [chrom, start, end, str(tdit[peirod])] ) )
                if 0 :
                    trick.dinit( dit, peirod, 'region', [] )
                    trick.dinit( dit, peirod, 'val', [] )
                    dit[peirod]['region'].append('\t'.join( [chrom, start, end ]))
                    dit[peirod]['val'].append( str(tdit[peirod]) )
    
    for peirod in dit:
        beds = dit[peirod]
        #vals = ','.join( list( dit[peirod].values() ) )
        if 0 :
            beds = dit[peirod]['region']
            vals = ','.join( dit[peirod]['val'] )
        bed = BED.bed( beds ).merge()
        intersect = BED.bed( bed ).intersect( beds )
        for line in intersect:
            line_arr = line.strip().split( '\t' )
            key = BED.bed( line_arr[0:3] ).view()
            match_fragment = BED.bed( line_arr[3:6] ).view()
            val = float( line_arr[-2] )
            trick.dinit( pdit, peirod, key, [])
            pdit[peirod][key].append( [ match_fragment, val] )
    for peirod in pdit:
        for key in pdit[peirod]:
            max_pos = sorted( pdit[peirod][key], key = lambda x:x[1], reverse = True )
            out += '\t'.join([ peirod, key, ','.join( [str(i[1]) for i in max_pos ] ) ] )+'\n'
            out += ','.join( [ ' '.join([ str(j) for j in i ]) for i in max_pos ] ).strip() +'\n\n'
    return out

def bedGraph( chrom, coord, values, rs, filename, cut):
    fh = open( filename, 'w')
    dit = {'chr':[],'start':[],'end':[]}
    ...
    return tab_postive_regions( tab, cut )
...
def already():
    ...
    for tab in tabs:
        ...
        if len(lst) == args.span * 2 + 2 :
            tregion = (lst[ args.span + 1 ].strip().split('\t')[0:3])
            infor[ gene ].append(tregion)
            sys.stderr.write('!!!Waring: already for %s,%s\n' % (gene, ','.join(tregion)))
    return infor
...
if __name__ == '__main__':
    ...
    dit = make_signal(dirs, quiet, needLoadChroms)
    ...
    for i,line in enumerate(fh):
        ...
        postive_tab = tab.replace('.tab','.pdf.txt')
        #pdf = matplotlib.backends.backend_pdf.PdfPages( pdf )
        postive_regions = bedGraph( chrom, coord, pvalues, rs, tab, args.cut)
        fh = open( postive_tab, 'w')
        fh.write( postive_regions )
        fh.close()
        ...
        ax = df.plot( style='.-' )
        #ax.set_ylim(0, 10)
        ax.axvline( x= args.span, linestyle = '-.')
        ax.axhline( y = 3, linestyle = '-.')
        #f1 = plt.figure( 1 )
        #f2 = plt.figure( 2 )
        #f1.set_size_inches( 8, 6 )
        #f2.set_size_inches( 8, 100 )
        #f2.text( 10, 0, postive_regions)
